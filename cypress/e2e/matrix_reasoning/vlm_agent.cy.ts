import matrixReasoningVlmAgent from '../../support/agents/matrixReasoningVlmAgent';
import {
...
describe(`Matrix Reasoning — VLM agent (${provider})`, () => {
  ...
  let emptyStreak = 0;
  const EMPTY_DONE = 20;
  // Items the VLM has already answered (keyed by stimulus+choices). Gate-escape:
  ...
  function finalize(): void {
    const ts = Date.now();
    cy.task('writeJsonl', { path: `cypress/logs/vlm_matrix_${provider}_${ts}.jsonl`, records });
  ...
  function handleItem(i: number, win: TaskWindow): void {
    ...
    const key = `${stimulusAlt}::${choices.join('|')}`;
    ...
    if (answeredItems.has(key)) {
      cy.get('body', { log: false }).then(($b) => {
        if ($b.find(CHOICE_BUTTON).length > 0) cy.chooseMatrixOption(hasKey ? keyedIndex : 0);
      });
  ...
  it('benchmarks pattern completion via the VLM, scored against the app key', () => {
    resetAudioCapture();
    launchTask({
